...
async function showGithubStars(github) {
    ...
    let githubRepoData = await fetch(`https://img.shields.io/github/stars/${url}`)
    let parser = new DOMParser();
    let htmlDoc = parser.parseFromString(await githubRepoData.text(), 'text/html');
    let stars = htmlDoc.body.querySelector('text').innerHTML;
    return stars;
}
...
export default function PublicationCell({ data }) {
    ...
    return (
        <Card sx={{ minWidth: 275, margin: "2.5em" }}>
            <CardContent>
                <Typography sx={
                    {
                        "fontSize": "28px",
                ...
                <Box sx={{ '& button': { marginRight: '1em' } }}>
                    ...
                    {data.link.github && (
                        <Button size="small" endIcon={<GitHubIcon />} onClick={() => window.open(data.link.github)}>
                            {"github"}
                        </Button>
                    )}
                    {data.link.github && (
